...
if [ "$STEP" = all ] || [ "$STEP" = 4 ]; then
  echo "=== ④ 41回路をスーパーセットに段固定配置 (JOBS=$JOBS ATIME=$ATIME) ==="
  CONE=$RES/superset_noskip.v
  [ -f "$CONE" ] || { echo "$CONE が無い。STEP=3 を先に"; exit 1; }
  if [ $# -gt 0 ]; then CKTS="$*"; else CKTS=$(ls "$RES/eblif_relay"); fi
  one() {
    c=$1
    (cd "$SRC" && ATIME=$ATIME python3 place_fixed_skip_ext.py "$RES/eblif_relay/$c/mapped_$c.v.eblif" "$CONE") > "$RES/noskip_place/$c.log" 2>&1
    r=$(grep -m1 "^結果:" "$RES/noskip_place/$c.log" | sed 's/^結果: //')
    echo "$c  $r"
  }
  export -f one; export SRC RES ATIME CONE
