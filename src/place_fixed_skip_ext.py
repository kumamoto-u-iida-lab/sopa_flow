#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""place_fixed_skip_ext.py — place_fixed_skip.py の複製＋外部入力(EXT)の段別枠チェック版。
...
for s, u in edges1:
    cu = col_of[u]
    for r in range(widths[cu]):
        allowed = cand[cu][r]
        b = m.NewBoolVar('')
        m.Add(row[u] == r).OnlyEnforceIf(b)
        m.Add(row[u] != r).OnlyEnforceIf(b.Not())
        if allowed:
            bvs = []
            for rp in allowed:
                bb = m.NewBoolVar('')
                ...
                bvs.append(bb)
            m.AddBoolOr(bvs).OnlyEnforceIf(b)
        else:
            m.Add(b == 0)
...
if sv.StatusName(st) in ('FEASIBLE', 'OPTIMAL'):
    json.dump({'col': col_of, 'row': {o: sv.Value(row[o]) for o in cbo}},
              open(f'place_fixed_skip_ext_{CKT}.json', 'w'), ensure_ascii=False)
    print(f"  保存: place_fixed_skip_ext_{CKT}.json")
