"""
   ...
   用途     :   ILPの結果からマッピング後のネットリストを作成する
   使用法   :   python3 [subjectgraph_alu4.xml] [4-PAE.xml] [ILPマッピング済みの.jsonファイル]   
   作成者   :   iwasaki ryo
   作成日   :   2024 / 10 / 30
...
def get_args(argv):
    ...
    parser.add_argument("json_file", help="Path to the .json file")  # 新しく追加した.jsonファイルの引数
...
def convert_cell_base_netlist(select_result, subjectgraph_dict, patterngraph_dict):
    ...
    cell_netlist = {subjectgraph_name: {patterngraph_name: [] for patterngraph_name in patterngraph_dict.keys()} for subjectgraph_name in subjectgraph_dict.keys()}

    for select_subject_name, select_patterns in select_result.items():

        if select_subject_name in subjectgraph_dict:
            target_subject_name = select_subject_name
        elif len(subjectgraph_dict) == 1:
            target_subject_name = next(iter(subjectgraph_dict.keys()))
            print(f"WARNING: select_result key '{select_subject_name}' does not match any subjectgraph; using '{target_subject_name}' instead.")
        else:
            print(f"WARNING: select_result key '{select_subject_name}' not found in subjectgraph_dict; skipping.")
            continue

        target_subject_obj = subjectgraph_dict[target_subject_name]

        print(f"DEBUG: Converting for subjectgraph '{select_subject_name}' -> '{target_subject_name}'")

        for patterngraph_name, graph_list in select_patterns.items():

            if patterngraph_name not in patterngraph_dict:
                print(f"WARNING: patterngraph '{patterngraph_name}' not found; skipping.")
                continue

            if patterngraph_name not in cell_netlist[target_subject_name]:
                cell_netlist[target_subject_name][patterngraph_name] = []

            convert_graph_to_cell(graph_list, cell_netlist[target_subject_name][patterngraph_name], target_subject_obj, patterngraph_dict[patterngraph_name])
...
import json
def convert_double_to_single(file):

    #jsonファイルの読み込み
    with open(file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    #ダブルクォーテションをシングルクォーテーションに変換
...
def main(argv):
    ...
    techmap_result = convert_double_to_single(args.json_file)
    select_result = {'top':techmap_result}
    print("3")
    
    ### 5.セルベースのネットリストに変換 ###

    cell_netlist = convert_cell_base_netlist(select_result, subjectgraph_dict, patterngraph_dict)
    print("4")

    ### 6.extended Bilf形式でマッピング結果を出力 ###

    # write_blif(cell_netlist, subjectgraph_dict, cell, patterngraph_dict)
    write_eblif(cell_netlist, subjectgraph_dict, cell, patterngraph_dict)
    print("5")
